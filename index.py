...
class InvertedIndexReader(InvertedIndex):
    ...
    def __next__(self):
        ...
        return term, postings_list, tf_list
    ...
    def get_postings_list(self, term):
        ...
        encoded_postings = self.index_file.read(length_in_bytes_of_postings_list)
        postings_list = self.postings_encoding.decode(encoded_postings)

        # loncat ke posisi byte tertentu pada file (index file) dimana list of TF disimpan
        self.index_file.seek(start_pos + length_in_bytes_of_postings_list)
        encoded_tf = self.index_file.read(length_in_bytes_of_tf_list)
        tf_list = self.postings_encoding.decode_tf(encoded_tf)
...
class InvertedIndexWriter(InvertedIndex):
    ...
    def append(self, term, postings_list, tf_list):
        ...
        self.doc_length.update({
            doc_id: self.doc_length.get(doc_id, 0) + tf_list[postings_list.index(doc_id)]
